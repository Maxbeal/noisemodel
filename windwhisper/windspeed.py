...
def load_wind_speed_data(filepath: Path, array: xr.DataArray = None) -> xr.DataArray:
    """
    Load the wind speed data from a file, or from a xarray.DataArray if provided.
    :param filepath: Filepath to the wind speed data.
    :param array: A xarray.DataArray containing the wind speed data.
    ...
    if array is not None:
        ...
        return xr.open_dataset(filepath).to_array().mean(dim="month")
    ...
    def __init__(
        ...
        try:
            self.wind_speed = load_wind_speed_data(
                filepath=FIXTURE_DIR / "era5_mean_2013-2022_month_by_hour.nc",
